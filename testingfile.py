...
class MagnitudeModel(object):
    ...
    def computeMagnitude(self):
        upper = np.sum(self.sampledFlux*self.samplingWavelength*self.sampledTransCurve)
        lower = np.sum(self.sampledFluxAB*self.samplingWavelength*self.sampledTransCurve)
        print(upper/lower)
